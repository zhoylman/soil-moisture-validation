...
out_dir = "/mnt/data1/soil-moisture-models/nldas2/noah"
...
files = list.files('/mnt/data1/soil-moisture-models/nldas2/noah/', full.names = T)
files_short = list.files('/mnt/data1/soil-moisture-models/nldas2/noah/', full.names = F)

for(i in 1:length(files)){
  print(files_short[i])
  temp_terra = rast(files[i])
  temp_root = temp_terra$SoilM_0_100cm
  writeRaster(temp_root, paste0('/mnt/data1/soil-moisture-models/nldas2/noah_0-100cm_sm/', files_short[i], '_0-100cm_sm.tif'))
}


#write out nc in chunks terra cant write out nc's in +1000 layers it doesnt seam (wierd errors)
num_files = list.files('/mnt/data1/soil-moisture-models/nldas2/noah_0-100cm_sm') %>% length()
...
for(i in 2:length(nc_breaks)){
  ...
  nldas2_rasterStack = list.files('/mnt/data1/soil-moisture-models/nldas2/noah_0-100cm_sm', full.names = T)[group_index] %>%
    purrr::map(., raster) %>%
    stack() %>%
    rast()
  ...
  write_csv(time, paste0('/mnt/data1/soil-moisture-models/nldas2/temp/noah_time/noah_time_group_', i-1 , '.csv'))
  writeCDF(nldas2_rasterStack, paste0('/mnt/data1/soil-moisture-models/nldas2/temp/noah_nc/nldas2_noah_soil_moisture_0-100cm_group_', i-1 ,'.nc'), overwrite = T)
}

#finally, merge datasets with cdo and write time
nc_files = tibble(nc_path = list.files('/mnt/data1/soil-moisture-models/nldas2/temp/noah_nc', pattern = '.nc', full.names = T),
                  nc_name = list.files('/mnt/data1/soil-moisture-models/nldas2/temp/noah_nc', pattern = '.nc', full.names = F),
                  time_path = list.files('/mnt/data1/soil-moisture-models/nldas2/temp/noah_time', pattern = '.csv', full.names = T),
                  time_name = list.files('/mnt/data1/soil-moisture-models/nldas2/temp/noah_time', pattern = '.csv', full.names = T)) %>%
...
command = paste0('cdo mergetime', paste0(' ', nc_files$nc_path, collapse = ''),
                 paste0(' /mnt/data1/soil-moisture-models/nc/NLDAS2_NOAH_soil_moisture_0-100cm.nc'),
                 collapse = '')
...
write_csv(time, '/mnt/data1/soil-moisture-models/nc/NLDAS2_NOAH_soil_moisture_0-100cm_time.csv')
